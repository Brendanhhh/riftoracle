...
def parse_match_payload(payload: dict[str, Any]) -> MatchExample:
    ...
    blue = _team_from_participants(participants, team_id=100, team_win_flag=team_outcome.get(100, False))
    red = _team_from_participants(participants, team_id=200, team_win_flag=team_outcome.get(200, False))
...
def iter_match_files(matches_dir: Path) -> Iterable[Path]:
    yield from matches_dir.rglob("*.json")
